...
type Options struct {
	ResIdSize       int64 //size in bits e.g. 32 bit, 24 bit.
	Mode            DrsmMode
	ResourceValidCb func(int64) bool // return if ID is in use or not used
...
type DrsmInterface interface {
	...
	FindOwnerInt64ID(id int64) (*PodId, error)
...
func (d *Drsm) AllocateInt64ID() (int64, error) {
	mutex.Lock()
	defer mutex.Unlock()
...
func (d *Drsm) ReleaseInt64ID(id int64) error {
	mutex.Lock()
	defer mutex.Unlock()
	if d.mode == ResourceDemux {
...
func (d *Drsm) FindOwnerInt64ID(id int64) (*PodId, error) {
