...
func (d *Drsm) punchLiveness() {
	...
	for {
		...
		case <-ticker.C:
			...
			update := bson.D{{Key: "_id", Value: d.clientId.PodName}, {Key: "type", Value: "keepalive"}, {Key: "podIp", Value: d.clientId.PodIp}, {Key: "podId", Value: d.clientId.PodName}, {Key: "podInstance", Value: d.clientId.PodInstance}, {Key: "expireAt", Value: timein}}
...
func (d *Drsm) addPod(full *FullStream) *podData {
	...
	pod.podChunks = make(map[int64]*chunk)
